...
int main() {
  try {
    ...
    // =========================================================================
    {
      ...
      pulsar::EnvState env{};
      env.cars.push_back(car);
      pulsar::AgentRewardState agent_state{};
      pulsar::EnvRewardState env_state{};
      auto bd = engine.compute(0, car, env, 2, agent_state, env_state, false, 0);
      require_close(bd.gameplay, 0.0f, "ball touch no hand-coded gameplay reward");
      require_close(bd.total, 0.0f, "ball touch no hand-coded total reward");
      std::vector<pulsar::AgentRewardState> previous{agent_state};
      std::uint8_t events[pulsar::kSparseEventChannelCount]{};
      engine.detect_sparse_events(0, car, env, 0, previous, agent_state, env_state, false, 0, events);
      require(events[pulsar::sparse_event_channel_index("ball_touch")] != 0, "ball touch event set");
    }

    // =========================================================================
    // 6. Flip reset event - transition detection
    // =========================================================================
    {
      auto cfg = make_reward_test_config();
      pulsar::RewardEngine engine(cfg);
      ...
      // Step 1: No flip reset
      car.has_flip_reset = false;
      auto bd1 = engine.compute(0, car, env, 2, agent_state, env_state, false, 0);
      require_close(bd1.mechanic, 0.0f, "no flip reset mechanic");
      std::vector<pulsar::AgentRewardState> previous{agent_state};
      env.cars[0] = car;
      engine.detect_sparse_events(0, car, env, 0, previous, agent_state, env_state, false, 0, events);
      require(agent_state.prev_has_flip_reset == false, "prev_has_flip_reset is false");

      // Step 2: Transition to has_flip_reset = true
      car.has_flip_reset = true;
      auto bd2 = engine.compute(1, car, env, 2, agent_state, env_state, false, 0);
      require_close(bd2.mechanic, 0.0f, "flip reset no hand-coded mechanic reward");
      require_close(bd2.total, 0.0f, "flip reset no hand-coded total reward");
      previous = {agent_state};
      env.cars[0] = car;
      engine.detect_sparse_events(1, car, env, 0, previous, agent_state, env_state, false, 0, events);
      require(events[pulsar::sparse_event_channel_index("flip_reset")] != 0, "flip reset event set");
      require(agent_state.prev_has_flip_reset == true, "prev_has_flip_reset is true");

      // Step 3: Continues having flip reset, no new transition
      auto bd3 = engine.compute(2, car, env, 2, agent_state, env_state, false, 0);
      require_close(bd3.mechanic, 0.0f, "no transition flip reset mechanic");
      previous = {agent_state};
      engine.detect_sparse_events(2, car, env, 0, previous, agent_state, env_state, false, 0, events);
      require(events[pulsar::sparse_event_channel_index("flip_reset")] == 0, "flip reset event not repeated");
      require(agent_state.prev_has_flip_reset == true, "prev_has_flip_reset is true");

      // Step 4: Loses flip reset
      car.has_flip_reset = false;
      auto bd4 = engine.compute(3, car, env, 2, agent_state, env_state, false, 0);
      require_close(bd4.mechanic, 0.0f, "lose flip reset mechanic");
      previous = {agent_state};
      env.cars[0] = car;
      engine.detect_sparse_events(3, car, env, 0, previous, agent_state, env_state, false, 0, events);
      require(agent_state.prev_has_flip_reset == false, "prev_has_flip_reset is false");
    }
