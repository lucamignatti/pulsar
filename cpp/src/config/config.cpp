...
void from_json(const json& j, ReplayBufferConfig& value) {
  reject_removed_section(j, "capacity");
  reject_removed_section(j, "min_fill_before_sampling");
  reject_removed_section(j, "segment_length");
  reject_removed_section(j, "her_relabel_fraction");
  reject_removed_section(j, "her_future_k");
  reject_removed_section(j, "goal_reach_epsilon");
  reject_removed_section(j, "epsilon_position_only");
  value.max_episodes = j.value("max_episodes", 2048);
...
void to_json(json& j, const CRLConfig& value) {
  j = json{
      {"enabled", value.enabled},
      {"car_head_initial_weight", value.car_head_initial_weight},
      {"car_head_anneal_steps", value.car_head_anneal_steps},
      {"num_updates_per_rollout", value.num_updates_per_rollout},
      {"car_goal_dim", value.car_goal_dim},
  };
}

void from_json(const json& j, CRLConfig& value) {
  value.enabled = j.value("enabled", true);
  value.car_head_initial_weight = j.value("car_head_initial_weight", 1.0F);
  value.car_head_anneal_steps = j.value("car_head_anneal_steps", 1.0e9F);
  value.num_updates_per_rollout = j.value("num_updates_per_rollout", 4);
  value.car_goal_dim = j.value("car_goal_dim", 3);
}
...
void from_json(const json& j, ExperimentConfig& value) {
  ...
  reject_removed_section(j, "sac_critic");
  reject_removed_section(j, "pbrs");
  reject_removed_section(j, "subgoal_planner");
  // self_play_league and world_model were removed; silently ignore old config files
  value.schema_version = j.value("schema_version", 7);
  ...
  value.crl = j.value("crl", CRLConfig{});
}
...
void validate_experiment_config(const ExperimentConfig& config) {
  ...
  if (config.replay_buffer.max_episodes <= 0) {
    throw std::invalid_argument("replay_buffer.max_episodes must be positive.");
  }
  if (config.replay_buffer.max_episode_length <= 0) {
    throw std::invalid_argument("replay_buffer.max_episode_length must be positive.");
  }
  if (config.replay_buffer.min_episodes_before_sampling <= 0) {
    throw std::invalid_argument("replay_buffer.min_episodes_before_sampling must be positive.");
  }
  if (config.crl.car_goal_dim <= 0) {
    throw std::invalid_argument("crl.car_goal_dim must be positive.");
  }
  if (config.crl.num_updates_per_rollout <= 0) {
    throw std::invalid_argument("crl.num_updates_per_rollout must be positive.");
  }
}
