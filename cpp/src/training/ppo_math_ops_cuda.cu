...
at::Tensor compute_q_boosted_gae_cuda(
...
at::Tensor compute_sparse_event_soon_targets_cuda(
    const at::Tensor& sparse_events,
    const at::Tensor& dones,
    const at::Tensor& horizons) {
  const c10::cuda::CUDAGuard guard(sparse_events.device());
  ...
  constexpr int kThreads = 256;
  sparse_event_soon_targets_kernel<<<blocks_for(steps * agents * channels, kThreads), kThreads, 0, at::cuda::getCurrentCUDAStream()>>>(
