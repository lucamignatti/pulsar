...
void Car::_FinishPhysicsTick(const MutatorConfig& mutatorConfig) {
	...
	{ // Limit velocities
		...
		constexpr float CAR_MAX_SPEED_BT = CAR_MAX_SPEED * UU_TO_BT;
		constexpr float CAR_MAX_SPEED_BT_SQ = CAR_MAX_SPEED_BT * CAR_MAX_SPEED_BT;
		constexpr float CAR_MAX_ANG_SPEED_SQ = CAR_MAX_ANG_SPEED * CAR_MAX_ANG_SPEED;

		if (vel.length2() > CAR_MAX_SPEED_BT_SQ)
			vel = vel.normalized() * CAR_MAX_SPEED_BT;

		if (angVel.length2() > CAR_MAX_ANG_SPEED_SQ)
