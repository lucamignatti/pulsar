...
void Arena::_BtCallback_OnCarCarCollision(Car* car1, Car* car2, btManifoldPoint& manifoldPoint) {
	...
	for (int i = 0; i < 2; i++) {
		...
		const CarState& state = car1->_internalState;
		const CarState& otherState = car2->_internalState;
		...
		Vec statePos = car1->_rigidBody.getWorldTransform().m_origin * BT_TO_UU;
		Vec otherStatePos = car2->_rigidBody.getWorldTransform().m_origin * BT_TO_UU;
		Vec deltaPos = (otherStatePos - statePos);

		Vec stateVel = car1->_rigidBody.m_linearVelocity * BT_TO_UU;
		Vec otherStateVel = car2->_rigidBody.m_linearVelocity * BT_TO_UU;

		if (stateVel.Dot(deltaPos) > 0) { // Going towards other car

			Vec velDir = stateVel.Normalized();
			Vec dirToOtherCar = deltaPos.Normalized();

			float speedTowardsOtherCar = stateVel.Dot(dirToOtherCar);
			float otherCarAwaySpeed = otherStateVel.Dot(velDir);
...
Arena::Arena(GameMode gameMode, const ArenaConfig& config, float tickRate) : _mutatorConfig(gameMode), _config(config) {
	...
	{ // Initialize world
		...
		if (_config.useCustomBroadphase) {
			_cachedBroadphase = static_cast<btRSBroadphase*>(_bulletWorldParams.broadphase);
		} else {
			_cachedBroadphase = nullptr;
		}
...
void Arena::Step(int ticksToSimulate) {
	...
	const bool isSoccar = (gameMode == GameMode::SOCCAR);
	const bool isDropshot = (gameMode == GameMode::DROPSHOT);
	...
	for (int i = 0; i < ticksToSimulate; i++) {
		...
		if (isSoccar) {
			bool all_safe = true;
			btVector3 ball_pos_bt = ball->_rigidBody.getWorldTransform().getOrigin();
			constexpr float SAFE_X_BT = 2800.f * UU_TO_BT;
			constexpr float SAFE_Y_BT = 3800.f * UU_TO_BT;
			constexpr float SAFE_Z_BT = 1850.f * UU_TO_BT;
			if (std::abs(ball_pos_bt.x()) >= SAFE_X_BT || std::abs(ball_pos_bt.y()) >= SAFE_Y_BT || ball_pos_bt.z() >= SAFE_Z_BT) {
				all_safe = false;
			}
			if (all_safe) {
				for (Car* car : _cars) {
					btVector3 car_pos_bt = car->_rigidBody.getWorldTransform().getOrigin();
					if (std::abs(car_pos_bt.x()) >= SAFE_X_BT || std::abs(car_pos_bt.y()) >= SAFE_Y_BT || car_pos_bt.z() >= SAFE_Z_BT) {
						all_safe = false;
						break;
					}
				}
			...
			if (_cachedBroadphase) {
				_cachedBroadphase->skipStatic = all_safe;
			}
		}
