...
void Ball::_FinishPhysicsTick(const MutatorConfig& mutatorConfig) {
	...
	{ // Limit velocities
		...
		float ballMaxSpeedBTSq = ballMaxSpeedBT * ballMaxSpeedBT;
		if (vel.length2() > ballMaxSpeedBTSq)
			vel = vel.normalized() * ballMaxSpeedBT;

		constexpr float BALL_MAX_ANG_SPEED_SQ = BALL_MAX_ANG_SPEED * BALL_MAX_ANG_SPEED;
		if (angVel.length2() > BALL_MAX_ANG_SPEED_SQ)
...
bool Ball::IsSphere() const {
	return _collisionShape->getShapeType() == SPHERE_SHAPE_PROXYTYPE;
}
