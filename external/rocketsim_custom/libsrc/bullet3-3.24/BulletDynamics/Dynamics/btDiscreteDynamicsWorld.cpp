...
void btDiscreteDynamicsWorld::internalSingleStepSimulation(btScalar timeStep)
{
	...
	// calculateSimulationIslands(); // Custom Optimization: Bypassed to avoid island graph building overhead
...
void btDiscreteDynamicsWorld::solveConstraints(btContactSolverInfo& solverInfo)
{
	...
	// Reset companionIds and hitFractions for all non-static rigid bodies in the world
	for (int i = 0; i < m_nonStaticRigidBodies.size(); i++)
	{
		m_nonStaticRigidBodies[i]->setCompanionId(-1);
		m_nonStaticRigidBodies[i]->setHitFraction(1.f);
	}
	...
	// Custom Optimization: Direct single flat batch solve to bypass island graph building
	int maxNumManifolds = getCollisionWorld()->getDispatcher()->getNumManifolds();
	btPersistentManifold** manifoldsPtr = maxNumManifolds ? getCollisionWorld()->getDispatcher()->getInternalManifoldPointer() : nullptr;

	m_solverIslandCallback->processIsland(
		m_collisionObjects.size() ? &m_collisionObjects[0] : nullptr,
