...
public:
	...
	// Debug counters commented out for performance
	...
	std::vector<btRSBroadphaseProxy*> m_dynProxiesList;
		...
		void RemoveStatic(btRSBroadphaseProxy* proxy) {
			for (int i = 0; i < staticHandles.size(); i++) {
				if (staticHandles[i] == proxy) {
					staticHandles[i] = staticHandles.back();
					staticHandles.pop_back();
					return;
				}
			}
		}
